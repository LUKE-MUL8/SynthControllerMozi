...
// Global parameters
int attack = 10; // Attack time in milliseconds
int decay = 20; // Decay time in milliseconds
int sustain = 128; // Sustain level (0-255)
int release = 50; // Release time in milliseconds
int filterCutoff = 255; // Filter cutoff (not used in this example)
int detune = 0; // Detune amount (not used in this example)
int transpose = 0; // Transpose amount
int octaveShift = 2; // Octave shift
...
void setup() {
  ...
  for (int i = 0; i < NUM_VOICES; i++) {
    ...
    envelopes[i].setADLevels(255, 128); // Set attack and decay levels
    envelopes[i].setTimes(attack, decay, sustain, release); // Set envelope times
    voices[i].note = 0;
